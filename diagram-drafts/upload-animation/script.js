...
// file appears as a single vertical rectangle
function step1() {
  for (let i=0; i < stripesPerSegment; i++) {
    for (let j=0; j < erasureSharesPerStripe; j++) {
      let shareEl = $('[stripe="' + i + '"][piece="' + j + '"]');
      let width = finalShareSize / 3;
      let height = finalShareSize;
      let topOffset = finalShareSize / 2;
      let top = topOffset + i * finalShareSize;
      let leftOffset = (sandboxWidth - finalShareSize) / 2;
      let left = leftOffset + j * width;

      shareEl.css('width', width + 'px');
      shareEl.css('height', height + 'px');
      shareEl.css('top', top + 'px');
      shareEl.css('left', left + 'px');

      if (j < paritySharesPerStripe) {
        let parityShare =  $('[stripe="' + i + '"][piece="' + (j + erasureSharesPerStripe) + '"]');
        parityShare.css('width', width + 'px');
        parityShare.css('height', height + 'px');
        parityShare.css('top', top + 'px');
        parityShare.css('left', left + 'px');
        parityShare.removeClass('visible');
      }
    }
  }

  $('#description').text('We start with a shard (a portion of an encrypted file).')
}

// file splits into stripes, which appear as horizontal rectangles
function step2() {
  for (let i=0; i < stripesPerSegment; i++) {
    for (let j=0; j < erasureSharesPerStripe; j++) {
      let shareEl = $('[stripe="' + i + '"][piece="' + j + '"]');
      let width = finalShareSize;
      let height = finalShareSize;
      let topOffset = finalShareSize / 2;
      let top = topOffset + i * finalShareSize + i * height;
      let leftOffset = sandboxWidth - erasureSharesPerStripe * finalShareSize;
      leftOffset /= 2;
      let left = leftOffset + j * width;
  ...
  $('#description').text('Then, we split the shard into several "stripes". In this example, there are three stripes.')
}

// stripes split into erasure shares, each appearing as a square
function step3() {
  for (let i=0; i < stripesPerSegment; i++) {
    for (let j=0; j < erasureSharesPerStripe; j++) {
      let shareEl = $('[stripe="' + i + '"][piece="' + j + '"]');
      ...
      let top = topOffset + i * finalShareSize + i * height;
      let leftOffset = sandboxWidth - erasureSharesPerStripe * finalShareSize;
      leftOffset -= (erasureSharesPerStripe - 1) * finalShareSize / 2;
      leftOffset /= 2;
      let left = leftOffset + j * width + j * width / 2;
  ...
  $('#description').text('Each stripe is divided into equally sized "erasure shares". In this example, each stripe has three erasure shares.')
}

// erasure shares change color to indicate that they have become encrypted. Reed-Solomon parity shares appear alongside normal erasure shares for each stripe
function step4() {
  let totalSharesPerStripe = erasureSharesPerStripe + paritySharesPerStripe;
  for (let i=0; i < stripesPerSegment; i++) {
    for (let j=0; j < totalSharesPerStripe; j++) {
      let shareEl = $('[stripe="' + i + '"][piece="' + j + '"]');
      let width = finalShareSize;
      let height = finalShareSize;
      let topOffset = finalShareSize / 2;
      let top = topOffset + i * finalShareSize + i * height;
      let leftOffset = sandboxWidth - totalSharesPerStripe * finalShareSize;
      leftOffset -= (totalSharesPerStripe - 1) * finalShareSize / 2;
      leftOffset /= 2;
      let left = leftOffset + j * width + j * width / 2;
      ...
      if (j < paritySharesPerStripe) {
        ...
        parityShare.css('top', top + 'px');
        parityShare.css('left', left + 'px');
        parityShare.addClass('visible');
      }
    }
  }

  $('#description').text('The erasure shares are run through the Reed Solomon algorithm to produce "parity shares" (maroon). In this example, a stripe has two parity shares. In order to reproduce the entire stripe, any three out of the five total shares (including the parity shares) are needed.')
}
// erasure shares move downwards towards their corresponding farmers one at a time. Every share that moves to a farmer combines vertically with shares from different stripes that have gone to the same farmer
function step5() {
...
let segmentSteps = [
  {
    ...
    move: function(i) {
      ...
      segment.css('width', width + 'px');
      segment.css('height', height + 'px');
      segment.css('top', top + 'px');
      segment.css('left', left + 'px');
    }
  }, {
    step: 1,
    move: function(i) {
...
let erasureShareSteps = [
  {
    step: 0,
    move: function(i, j) {
      let shareEl = erasureShares[i][j];
      let width = finalShareSize / 3;
      let height = finalShareSize;
      let topOffset = finalShareSize / 2;
      let top = topOffset + i * finalShareSize;
