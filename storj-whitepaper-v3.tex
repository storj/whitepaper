...
There are three important roles involved in uploading a file: the console, the 
satellite, and storage nodes.
...
\item The console will then choose an encryption key and starting nonce for
...
of this first segment. The console will:
  \begin{itemize}
...
\item The console will upload back to the satellite a \x{pointer}, which
  contains which storage nodes were
...
\item The console will then proceed with the next segment and continue to
...
\item The storage nodes, at some future point, will send the largest restricted
...
\item The console is going to try and reduce round trips to the satellite
...
\item The console is going to request all of the segment pointers for the file.
...
\item Each roll-up period, a satellite will consider all of the files it
  ...
  satellite believes a storage node has for that rollup period, 
the satellite will
...
previously missing shards are sent to storage nodes selected by the satellite.
