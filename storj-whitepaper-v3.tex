...
security and privacy. At a greater rate than any single entity could afford,
this allows market forces to optimize on cheaper ways to provide storage.
...
responsibilities and building such our distributed storage network.
...
is not as powerful or secure as these networks. One advantage of
building large networks is that it measurably improves security in how data is
stored.
As more people enter the network, it becomes more challenging to steal data.
...
concrete implementation of each component, while
...
could switch to Storj with only a
...
\todo{Maybe add a joke about some ridiculous failure case here}
...
Despite the chance of hardware failure, Maymounkov {\em et al}. found that in
...
TB/month cannot average more than 385 KB/s over the month without exceeding
...
recentralizing the system to some degree. So, in order to keep the storage
...
one large file.
As the protocol supports streaming, small files can even be
...
trying to exploit the suggested protocol to their advantage, or nodes that
...
{\em Rational} nodes, on the other hand, participate exactly when it is in
their net best interest to do so, and may depart for the same reason.
...
All peers on the network communicate via a ubiquitous standard protocol. The framework requires all
