...
Storj Labs, Inc.}
\date{\small\sffamily\color{white} v3.0, October 24, 2018\\
...
% TODO: remove DRAFT and update date when ready to launch
...
\vfill
{\footnotesize\noindent\new{
Copyright \textcopyright\ 2018 Storj Labs, Inc.\\
...
provide particular acknowledgement \new{for the guidance and inspiration provided
by} the teams that \new{designed and} built
Allmydata,
Ceph,
CoralCDN,
Ethereum,
...
frequency of \new{such} breaches has been increasing by as much as a factor of 10
...
\new{costing} less than current storage methods.
...
more economical than either \new{on-premise} or traditional, centralized cloud storage.
...
\ref{chap:concrete}
proposes a simple concrete implementation of \new{the framework},
...
\new{might have} access to less expensive electricity than standard data centers or another small
operator \new{could have} access to less expensive cooling. Many of these small operator
environments are not substantial enough to run an entire datacenter-like
storage system. For example, perhaps a \new{small business or home Network Attached
Storage (NAS)} operator has
enough excess electricity to run ten \new{hard drives}, but \new{not} more. We have found that
...
in 2018, and \new{is expected to} reach \$302.5 billion by 2021 \cite{gartner-cloud-growth}.
...
centralized cloud.
However, to \new{encourage} partners or customers to bring data to the network,
...
\item[Storage node operators] - It must be economically attractive for storage
  ...
  new capacity, \new{so that we can grow the network beyond the capacity that
  currently exists}.
...
providers \new{by alleviating many switching costs for our users.}
...
the asymmetric internet connections offered by \new{many} ISPs.
...
whereas an object store or \new{file system} is ideal for storing many large files.
...
database-like use cases, object storage is the predominant \new{initial} use case described in
...
where all nodes are considered {\em altruistic}. For example, \new{absent} hardware failure
...
Likewise, \new{the effects of byzantine behavior must be minimized or eliminated.}
...
much as is feasible, describe Storj both now and \new{ten} years from now.
...
\new{GFS \cite{gfs}, HDFS \cite{hdfs}, or} Lustre \cite{lustre}, this decision
...
by a unique identifier so that, given a peer's \new{unique identifier}, any other
...
\new{In contrast, replication requires significantly higher expansion factors for
...
To see how \new{these tables were} calculated, we'll start
...
Data \new{must} be encrypted as early as possible in the data storage pipeline,
...
mechanism \new{in cases where their encryption choices are changed or upgraded.}
...
In addition, the \new{framework} also tracks and aggregates the value of the
...
\item[Satellite] This peer class participates in the node discovery system,
  ...
  \new{Users have accounts on and trust specific Satellites.}
...
identified by paths. \new{Every} file has a unique path within a bucket.
...
bytes, and has no minimum or maximum size. A file is represented \new{by} an ordered
...
also heavily mirrors \new{distributed storage systems such as the Google File System
\cite{gfs} (and other GFS-like systems \cite{hdfs, qfs, hopsfs}) and} the
Lustre distributed \new{file system} \cite{lustre}. In \new{every case},
...
The storage node \new{is being} developed and \new{will be} released as open source software.
...
ordered delivery (like TCP would) \new{with LEDBAT \cite{ledbat} functionality};
...
cache lookup and will not attempt \new{another} lookup using Kademlia.
...
literature. \new{GFS refers to segments as chunks \cite{gfs}.}
...
The Linux \new{file system} Ext4 performs the same optimization
...
As in \new{other distributed storage systems \cite{gfs, hdfs, qfs, hopsfs, lustre}},
...
but a traditional database solution considerably increases the risk area from \new{using}
encryption keys. Fortunately, the metadata itself can be periodically backed
up into the Storj network. \new{This in turn allows us to only keep track of the
metadata of this metadata, decreasing the amount of critical information that
must be stored elsewhere even further.}
...
rearchitectures of these systems, \new{such as the removal of the Satellite
entirely}.
...
component.
Let's say a given path $\semibold{p}$ has \new{unencrypted} path components
$p_1, p_2, \ldots, p_n$
...
A key \new{$K(s_i)$} can be deterministically generated from $s_i$.
...
and access any arbitrary $e_4$, as $K(s_3)$ is known to them, \new{without
...
disallowing clients \new{from making unauthorized edits}.
...
in that they allow for rich contextual decentralized delegation. \new{In other words}, they
provide the property that anyone can add restrictions in a way in which those
restrictions cannot later be removed, \new{without coordination with a central party}.
...
Our macaroons are further caveated with \new{optional expirations and revocation
...
audits without storing all of the requested data \new{by keeping track of which
...
\new{behave rationally, and incentives are aligned such that}
...
mode, the Satellite will \new{calculate and save the expected response, then}
...
\new{All audit failures will be stored and saved in the reputation system.
Audits additionally serve as opportunity to test storage node latency,
throughput, responsiveness, and uptime. This data will also get saved in the
reputation system.}
...
the formerly missing pieces \new{are} sent back out.
...
Every time a file is uploaded, the \new{Satellite} will select a small number of
...
the \new{Satellite} will then start including that storage
...
completed, and the storage node may now enter other nodes' routing tables
...
and fail \new{{\em put} operations} without penalty, as nodes will be allowed to
choose which
...
of \new{accounting} to each storage node for every file
piece
it will be storing.
...
repairing \new{segments}, and storing metadata. Every day, each Satellite will execute
...
and for the repair,
once that \new{segment} falls below the piece threshold needed for
repair.

If a storage node \new{is detected to act}
...
\new{catch ``cheaters'' (or, rational nodes)}.
...
nodes with lots of garbage will earn less than they otherwise \new{would}
...
\new{our first release will start with a}
conservative garbage collection strategy,
\new{though we anticipate a precise strategy soon thereafter}.
...
the Uplink software in all three forms \new{is being} developed and \new{will be} released as
open source software.
...
We will retain \url{storj.io} as the place for \new{learning how to contribute} extra storage and
...
storage nodes are paid \new{fairly} and Satellites are able to continuously meet all
...
\item Return a list of nodes, along with their contact information and
  unrestricted bandwidth \new{allocations}, and a chosen root piece ID.
\end{itemize}
...
\item The Uplink will upload a {\em pointer} object back to the Satellite, which
  ...
  \item \new{the hash of the piece hashes}
  \item a signature
  \end{itemize}
\end{itemize}
...
publishing his own Paxos algorithm \cite{paxos}, \new{which became
...
disrupt network operation by hijacking or dropping messages. \new{While Kademlia
...
cases---than either \new{on-premise} storage solutions or traditional, centralized
...
capital or personnel. Running a drive at \new{full} capacity does not consume
significantly more power than running a drive \new{with excess space}. And, with
