...
storage protocols, we must create a graceful transition path from centralized
...
failures could happen at any time,
...
fines for customers who go over this limit. Such caps impose
...
storage system that is careless with bandwidth usage would be to give
...
object size. When storing lots of small bits of information, a
database is generally the preferred solution.
On the other hand, when storing lots of large
...
vast majority of objects stored will be a couple of megabytes or larger. It is
...
own purposes.
Storj operates in an environment where every node is potentially managed by
its own independent operator. In this environment we can expect that a majority
of storage nodes are rational and a minority are byzantine. Storj assumes no
altruistic nodes. Any potential design must account for this distinction.
...
to peers in the network and generates and stores some metadata about where to
find the data again.
...
One benefit from this approach is that it makes
updating concrete implementations of components within the framework much
easier without having to rearchitect the rest of the network. This is how
we will avoid yet another major rearchitecture.
...
deterministically as in a system like Kademlia \cite{kademlia}. This decision
implies the requirement of a separate metadata storage system.

\subsection{Peer-to-peer communication}

All peers on the network communicate via a ubiquitous standard protocol. The
framework requires all
...
implementation details, our framework is described in more general terms, while
...
failure between nodes is not correlated. This implies that erasure codes are an
