...
the security and privacy of object storage.
...
currently stored with a handful of techn companies that have the experience and
capital to build massive datacenters capable of handling this vast amount of
information.
In recent years, we have seen colossal infrastructure data breaches, periods of
unavailable on a grand scale, storage costs failing to decrease in line with
...
centralized datacenters. This model can address the rapidly
...
we are base our optimization efforts on.
...
significantly.
Our design constraints are heavily informed by our product and
market fit goals.
...
At bare minimum, this requires the methods sketched out in
...
system must encourage stable nodes and keep the churn rate as low as possible.
...
Decentralized storage systems have the potential to capitalize on
massive opportunities for parallelism compared to traditional data centers.
Some of these opportunities include increased transfer rates, processing
capabilities, and increased overall throughput even when individual
...
the latency of the link will be the lower bound for the overall operation.
...
Global bandwidth availability is increasing year after year; however, access to
...
fines for customers who go over this limit. \cite{comcast-cap}
...
centralizing the system to some degree. In order to keep the storage
...
For example, the United States has the Health Insurance Portability and
Accountability Act (HIPAA) which has requirements for data center
compatibility. Europe has General Data Protection Regulation (GDPR) regarding
...
centralized point of attack or failure and being implemented correctly.
...
cause it to sell your metadata to advertisers, or perhaps the company goes out
of business or otherwise fails to keep your data safe.
...
where all nodes are considered altruistic.

For example, Amazon owns all of their nodes directly and
...
\item provides complete privacy, where only the two peers
...
new pieces, we now need to keep track of which storage nodes we selected.
...
avoidance between users (\ref{sec:coordination-avoidance}).
...
risk, as evidenced by the findings of Maymonkov {\em et al.} that expected node
...
given our design constraints. However, within the framework there remains
...
Storage nodes will advertise during node discovery communications what hard
drive space is still
...
anything more complicated than store \x{pieces} directly on disk. Storage
nodes simply maintain a hierarchy of directories that contain the files they
store directly. Piece data is not stored in a key/value database. As a result,
unlike the previous release of Storj that used KFS \cite{storj-v2} (which
was a custom extension on top of LevelDB \cite{leveldb}), Storj no
...
paid for
storing more. Storage nodes are not paid for node discovery or other
maintenance traffic.

Storage nodes will support three methods: \code{get}, \code{put}, and
\code{delete}. They store {\em pieces}.
Each method will take a {\em piece ID}, a {\em satellite ID} (the ID of the
associated Satellite instance) and signature, an optional TTL,
and the other metadata required by the bandwidth allocation protocol (\ref{bap})
\todo{clean up the list of arguments}.
...
first.
Storage
nodes do not get penalized for rejecting initial \code{put} operations.
...
Over time, we'll be replacing TLS with a more flexible secure transport
...
(for example) can be done with only 3MB of memory, so the space requirements
are negligible.
...
\todo{talk about how we're using kademlia to advertise disk space, bandwidth
availability, and STORJ wallet address, and then storing that information in
the cache}
...
able to support arbitrarily named sets of keys/values improves
...
recommended that \x{segment} sizes be standardized as a byte multiple, such as
32 MB or 64 MB. Smaller \x{segment}s may be padded with zeroes or random data.
...
Segmenting large files like videos and distributing the \x{segment}s
...
The services that holds this metadata we call the \x{satellite}. Users of the
network will have accounts on a specific satellite instance, which will store
their metadata for them, manage authorization to data, keep track of storage
node quality via audits, repair and maintain data when redundancy is reduced,
and issue payments to storage nodes on the user's behalf.
Storj implements a thin-client model that delegates trust to the
satellite service which manages data ownership. Thin-clients (\x{uplinks})
...
if the data has been tampered with. Data encryption keys are chosen by clients
randomly.
...
trust over time with specific satellites that are likely to pay their bills.
Storage nodes
...
\todo{discuss quality control and branding more}
\todo{insurance}
