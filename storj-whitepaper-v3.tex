...
first get authorization from a payer 
\bs{who is the payer? client? heavy client? farmer?} 
that it has enough funds and is authorized
...
will start by sending a restricted allocation \bs{where?} 
for some small amount, perhaps
...
indicating that the client agreed to bandwidth usage of up to $y$
bytes, and the payer agreed that the client is allowed to use that
...
This is how we measure request-specific peer-to-peer bandwidth
