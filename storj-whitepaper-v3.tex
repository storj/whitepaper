...
\item Eng: section about identity leaf key rotation
...
providing an economical cloud storage solution to address the rapidly expanding
amount of data for which current solutions are cost-prohibitive.
With an anticipated 44ZB of data expected to exist by 2020 and a market that is
growing to \$92B in the same time frame, our point of view is that decentralized
cloud storage has the potential to address several key segments within that
market today, particularly related to long term archival storage.
...
object storage the content delivery networks (CDN).
...
While all these companies share a common core of building robust and secure
networks, they also share common principles of how it can be achieved.
Companies generate redundancy for data in case of failure, store this
...
cycle to address these challenges, but the evolution of decentralized storage
is subject to a specific set of design constraints.
These design constraints inform the overall requirements and implementation of
the network software components. When designing a distributed storage system,
there are a multitude of optimizations to make: speed, capacity, trustlessness,
...
breaches that have continuously plagued companies.
...
in which Storj operates and elaborates on specific design constraints that
represent the focus of what we plan to optimize for.
...
short. Most other cloud storage products provide some form of compatibility
with the S3 API.
...
failures could happen sporadically,
...
To address security concerns, files are always encrypted prior to leaving the data owners computer. The encryption used is widely used and proven secure. The libraries that are used are open-source, written by experts, and thoroughly audited.

\todo {Potentially add something about splitting up files in to pieces and spreading them out to Storage Nodes which increases privacy and security}

Privacy is achieved in a number of ways. First, all of the metadata associated with a file is encrypted so that any details about the data being uploaded is just as secure as the data itself. In addition, the sizes of data are standardized such that it is not viable to determine anything about data on the network by its size.
...
lots of files smaller than a megabyte. Such cases can be addressed with a
packing strategy, where many small files are aggregated and stored together as
one large file.
...
{\em Rational} nodes, on the other hand, participate or deviate only when it
...
potentially managed by its own independent operator.
...
\item[Storing data] When data is stored with the network, a client will encrypt
...
\item[Maintaining data] Data is maintained in the network by replacing
...
\item[Payments] A form of currency should be sent in exchange for services
rendered.
...
Aside from storing data, nodes should provide reliable storage space,
network bandwidth, and appropriate responsiveness.
In return, nodes are rewarded for their participation via payments.
...
responding accurately to audits, etc. Because storage
...
small amount of metadata and can't select nodes for storing data implicitly or
deterministically as in a system like Kademlia \cite{kad}. This decision
...
framework requires that this component:
...
ephemeral connection address. To achieve this, an {\em overlay network} can be
...
Once we have split an object up and selected storage nodes on which to store the
...
arbitrary key, often treated like a path, to identify this mapping of piece to
node. This implies the necessity of a metadata storage system.
...
files, arbitrarily large amounts of files, and so on. Metadata values
...
agreement is a non-starter. As a result,
...
The framework we've described we believe to be relatively fundamental given
our design constraints. However, within the framework there remains a
...
(\bs{where? add section reference}).
...
and gRPC provides multiplexing and a nice programmer interface.
...
should ideally be kept in cold storage to prevent key compromise.
...
are often on consumer internet behind routers with constantly changing IP
...
S/Kademlia extensions. \cite{skad}
...
on an ongoing basis, perhaps once per hour. The caching service will then cache
...
tails we can tolerate and is thus the amount of slow nodes we are immune to.
...
A \x{stripe} is no more than a couple of kilobytes, and erasure encoding a
single \x{stripe} at a time allows us to read portions of a large \x{segment}
...
failure between nodes is assumed to be not correlated.
This implies that erasure codes are an
extremely effective method of securing availability. Availability is
proportional to the number of nodes storing the data.
...
the problem of Byzantine distributed consensus.
...
We will describe a Distributed Repair method in the Future Works section, that
...
\item {\bf Satellite} - This peer class participates in the DHT, caches
  ...
  another satellite hosted by a trusted party like a friend, group, or
  workplace.
\item {\bf Uplink} - This peer class represents any application or
  service that wants to store data. Applications can store data via the
  S3-compatible console, or through our libstorj C-bindings. This peer class
  is not expected to remain online like the other two classes and is otherwise
  relatively lightweight. This peer class performs encryption, erasure encoding,
