...
\usepackage{hyperref} \def\UrlBreaks{\do\/\do-} % breaks long url in references
\usepackage{graphicx}
...
Storj Labs, Inc.}
\date{\small\sffamily\color{white} v3.0, October 5, 2018\\
...
\section{Durability, device failure, and churn}
...
\cite{dht-churn}, where participants join the network and then leave for
various reasons, well before their hardware has actually failed. The more
...
a tight relationship between network churn, additional redundancy (or,
the {\em expansion factor}), and bandwidth availability \cite{pick2-churn}. To
keep background bandwidth usage and redundancy low, our network must have
low network churn.

As an illustration, Maymounkov {\em et al.} found that in
...
See appendix \ref{appendix:RS} and work \cite{pick2-churn} for a discussion of
how repair bandwidth varies as a function of node churn.
...
to outlining the Storj framework, which consists of relatively fundamental
components. Here, we describe
...
much as is feasible, describe Storj both now and in 10 years from now.
...
(see section \ref{bap}), the client and storage node must be able
...
Erasure codes are well established in use for distributed and peer-to-peer
...
improve repair traffic significantly \cite{pick2-churn}.
...
Long-tail responses are so-named due to their average rare rate of occurrence
...
In MapReduce \cite{mapreduce}, long-tail responses are called ``stragglers,''
MapReduce executes ``backup tasks'' (additional, unnecessary work) to make sure
that if stragglers take too long, the overall operation can proceed without
waiting. If the backup task mechanism is disabled in MapReduce, basic operations
can take 44\% longer to complete, even though the backup task mechanism is
causing more work.
...
downloads can use the fastest peers to download the starting pieces and slower
...
so instead of implicit node selection such as a scheme using consistent
hashing, we must use an explicit node selection scheme such as directory-based
lookups \cite{p2p-policies}.
...
of data pieces to node. This features implies the necessity of a metadata
storage system.
...
As in \cite{proof-of-retrievability}, this auditing mechanism does not audit
...
risk, to the point where it is orders of magnitude larger than other causes.
...
to mere minutes \cite{dht-churn}. Storage nodes
...
that remains is to detect when a storage node goes offline or stops storing
data correctly and then repair the data it had to new nodes.
...
online for much longer than a few hours. Fortunately, our system includes
payments, so our payment strategy will involve rewarding storage node
...
In addition to the payment for services rendered to those contributing storage
and bandwidth, the system also tracks and aggregates the value of the
...
Though the Storj network is payment agnostic and
...
for payment. While we intend for the STORJ token to be the primary mechanism
indefinitely, in the future
other alternate payment types could be implemented, including Bitcoin,
...
Our overall strategy extends from our previous release \cite{storj-v2} and
also heavily mirrors the Lustre distributed filesystem \cite{lustre}. In
both Lustre and our system, there are three major actors in the network -
...
Our previous release used different names for each component. What we
...
compatible where possible, but in general we now refer to client software as
Uplinks.
...
satellite instance, and the other metadata required for bandwidth measurement
(see section \ref{bap}).
...
other, more available Nodes, for a fee.\footnote{See how we measure bandwidth
(section \ref{bap}) for a description of how fees work.} Nodes can also provide
...
The primary file data itself is simply referred to as a {\em stream}.
...
The highest level subdivision of a file's primary stream of data is called a
``segment.''
...
pre-generated. As we learned in our previous release, without a periodic
regeneration of these challenges, a Storage Node can begin to pass most
audits without storing all of the requested data. During our previous release,
we began to play with Reed-Solomon erasure coding and learned that it would
...
({\em stripes}), so we use it to read a single stripe at a time as a challenge
...
In the future, a distributed repair method may be constructed that
...
When repairing a segment, only the minimum number of pieces required will be
downloaded. Unfortunately, without full redundancy, erasure codes will be less
...
{\em pointer}. During repair, the hashes of every piece can be retrieved,
...
piece
it will be storing.
Satellites will track utilized bandwidth (see section \ref{bap}).
...
\subsection{Bandwidth allocation and exchange reporting}\label{bap}
...
between two peers. In our previous version \cite{storj-v2, sip9}, we used
{\em exchange reports} to gather information about what transpired between
two peers.

We extend exchange reports with a simple signing and incentive process to
more correctly verify that a certain amount of bandwidth has passed between two
peers. This system is inspired by Filecoin's off-chain retrieval market,
...
Uplink is the term which we use to identify any software or service that invokes Lib Uplink in order to interact with Satellites and Storage Nodes.

\subsection{Lib Uplink}

Lib Uplink is a library that provides access to storing and retrieving data in the
Storj network.

\subsection{Gateways}

Gateways act as compatibility layers between a service or application and the Storj network. They run as a service co-located with wherever data is generated, and will communicate directly with Storage Nodes so as to avoid central bandwidth costs. The Gateway is a simple service layer on top of {\em libuplink}.

Our first is an Amazon S3 gateway. It provides an S3-compatible, drop-in interface for users and
applications that
need to store data but don't want to bother with the complexities of distributed
storage directly.

\subsection{Uplink CLI}

The Uplink CLI is a command line application which implements Lib Uplink allowing its user to upload and download files, create and remove buckets, manage file permissions and other related tasks. It aims to provide an experience familiar to what you might expect when using Linux/UNIX tools such as scp or rsync.
...
Once all necessary segment pointers have been returned, if the requested
  ...
  while measuring bandwidth (section \ref{bap}) to all appropriate Storage
...
Fortunately, Satellites authorize all accesses to pieces already anyway, and
