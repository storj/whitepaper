...
S3 CompatibilityThis incorporates aggressive requirements for feature set, performance, and durability.
At a bare minimum, this requires the methods described in
Figure \ref{fig:s3-api-code} to be implemented.

While, as mentioned in this section, we are building a gateway with an S3 compatible API, we are also designing our native API to be simple, easy-to-integrate, and largely unconstrained by the Amazon S3 API.
...
storage -- to be interchangeable per user. Specifically, we expect the platform to incorporate
...
\item[Uplink] This peer class represents any application or
  ...
  Amazon S3-compatible API or through our {\em libstorj} C-bindings. This peer class
...
\item[Satellite] This peer class participates in the node discovery system,
  ...
  node reputation, pays Storage Nodes, aggregates billing data, performs audits and repair, and manages
...
Storj Labs partners who host Satellites ``Tardigrade'' Satellites may receive
a share of revenue greater than typical demand partners. The portion of the
revenue paid to Stoj Labs is intended to cover a range of different costs.
Such costs might include a payment continuity service. If the Satellite
temporarily goes offline, Storj Labs can continue to pay Storage Nodes on
behalf of the Satellite until the Satellite returns to avoid losing data.
Additionally, this fee may cover country-specific tax compliance functionality.
For example, this fee could cover assisting satellite operators in complying
with United States Form 1099 tax filing, which is required by United States
law for any payments to an individual in a given year exceeding a total
of \$600. Additional services are contemplated and will be included in
the roadmap as requirements are developed.
...
effects erasure code parameters.
...
as the reputation distribution is used not only directly as it pertains to uptime
...
Successes&Trials&Mean&Lower&Upper\\\hline
80&100&0.80&0.71&0.87 \\
85&100&0.85&0.76&0.91\\
90&100&0.90&0.82&0.95\\
95&100&0.95&0.89&0.98 \\
99&100&0.99&0.95&0.99 \\\hline
\end{tabular}
\end{table}
