...
sections. Chapter \ref{chap:design-constraints} discusses the design space
in which Storj operates and elaborates on specific design constraints which
we are focusing our optimization efforts on.
Chapter \ref{chap:framework} covers our framework, while \ref{chap:concrete}
proposes a simple concrete implementation of each component.
Chapter \ref{chap:walkthroughs} explains detailed descriptions of what happens
...
A growing body of distributed database system research shows that systems that
avoid coordination wherever possible perform far better than systems where
subcomponents are forced to coordinate to achieve correctness
\cite{cap1, cap2, consistency-vs-latency, hat, i-confluence, anna,
calm1, calm2}.
By coordination, we continue using Bailis {\em et al.}'s informal definition
...
\cite{i-confluence}.
This observation applies not only to distributed networks but also to
...
(e.g., operations that require linearizability
...
by Anna's performance successes, it is worth it to avoid coordination where
possible.

Systems that avoid too much coordination are much better at scaling from small
...
To get to exabyte scale, we believe that avoiding coordination where possible
is one of the key components of any successful strategy.
...
ledger. For us to achieve exabyte scale, we think it is a fundamental
...
agreement is a non-starter (\ref{sec:coordination-avoidance}).
...
that do not have a valid signature from the appropriate satellite. Storage
...
Storj implements a thin-client model that delegates trust around managing your files location metadata to the
...
network access. (say something here about Storage Node Satellite whitelists?)

With respect to customer data, the satellite is designed to store only
metadata. It is never given data or file metadata unencrypted and does not hold encryption keys.
The only knowledge of an object that the satellite is able to share with
third parties is its existence, rough size, and metadata such as access
patterns.
...
Applications using object stores invest significant amounts of trust in the
storage providers. Providers may choose to operate public satellites as a
service.
Application developers then delegate trust regarding the location of their data on the network to a specific satellite, as they
...
operational burdens from the application developer to the service-provider, in this case the storage nodes.
...
A specific satellite {\em instance}, or satellite service, does not necessarily constitute one
server. A satellite may be run as a collection of servers, or satellite instances and be backed by
