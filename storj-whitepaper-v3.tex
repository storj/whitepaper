...
Some distributed storage systems (e.g. Amazon S3, Azure Blob Storage)
...
long-lived source of truth and will be used directly in many situations.
...
our purposes, Amazon S3 supports ``object metadata'' \cite{s3-object-meta}. This ability to support arbitrarily named sets of keys-value pairs improves
...
Files will be designed for both small and large data.
...
This will also allow developers to pay for storage with standard payment
...
piece
it will be storing. The satellite
...
level, given the trade-off of additional bandwidth and faster collection.
...
Only after both pieces are deleted will node free the space.
...
providing Amazon S3 compatibility.
...
\chapter{Object Repair Costs}
	...
	As the saying goes, there is no free lunch. Erasure codes are pretty amazing, but they do come at a cost. The two largest being additional network bandwidth for object repair and extra space overhead (which we call the expansion factor). Consequently, a fundamental challenge is how to not only choose the system parameters that keep expansion and repair bandwidth to a minimum but also provide an acceptable level of durability. Fortunately, we are not alone in wondering about this, and there is a good amount of prior research on the problem. ``Peer-to-Peer Storage Systems: a Practical Guideline to be Lazy'' \cite{p2p-lazy} is an excellent guide, and much of our work follows from their conclusions. The end result is a mathematical framework which determines network durability and repair bandwidth given Reed-Solomon parameters, average node lifetime, and reconstruction rate. The following is a summary of results and explanation of their implications.
